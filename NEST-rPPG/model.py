...
# =============================================================================
# BaseNetAttn — BaseNet + TemporalAttention, no custom stem, no skip fusions
# Isolates the contribution of temporal attention alone.
# =============================================================================

class BaseNetAttn(nn.Module):
    """
    Ablation model: BaseNet encoder/decoder with TemporalAttention only.

    Differences vs ArcNet:
      - Stem:    original ResNet-18 7x7 conv (same as BaseNet, NOT TemporalAwareStem)
      - Decoder: plain upsampling (same as BaseNet, NO U-Net skip fusions)
      - Kept:    TemporalAttention between layer3 and layer4

    Forward API: forward(x) -> (Sig, HR, av)  identical to BaseNet / ArcNet.
    """
    ...
    def __init__(self, attn_heads: int = 8, attn_dropout: float = 0.1):
        ...
        # Original ResNet stem (no TemporalAwareStem)
    ...
    def forward(self, x: torch.Tensor):
        ...
        x  = self.layer1(x);  av1 = self.get_av(x)
        x  = self.layer2(x);  av2 = self.get_av(x)
        x  = self.layer3(x);  av3 = self.get_av(x)
...
base_net_attn = BaseNetAttn


# =============================================================================
# BaseNetAttnSkip — BaseNet stem + TemporalAttention + U-Net skip fusions
# Isolates contribution of attention + skip fusions without the custom stem.
# =============================================================================

class BaseNetAttnSkip(nn.Module):
    """
    Ablation model: original ResNet stem, TemporalAttention, U-Net skip fusions.

    Differences vs ArcNet:
      - Stem:    original ResNet-18 7x7 conv (NOT TemporalAwareStem)
      - Kept:    TemporalAttention between layer3 and layer4
      - Kept:    U-Net skip fusions in decoder

    Forward API: forward(x) -> (Sig, HR, av)  identical to ArcNet.
    """
    ...
    def __init__(self, attn_heads: int = 8, attn_dropout: float = 0.1):
        ...
        # Decoder with same structure as ArcNet
        self.up1 = nn.Sequential(
            nn.ConvTranspose2d(512, 512, kernel_size=[1, 2], stride=[1, 2]),
            BasicBlock(512, 256, [2, 1], downsample=1),
        )
        ...
        # U-Net skip merge convolutions (same as ArcNet)
        self.merge1 = nn.Sequential(
            nn.Conv2d(256 + 256, 256, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
...
base_net_attn_skip = BaseNetAttnSkip
...
class BaseNetResSkip(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x  = self.layer1(x);  av1 = self.get_av(x);  e1 = x
        x  = self.layer2(x);  av2 = self.get_av(x);  e2 = x
        x  = self.layer3(x);  av3 = self.get_av(x);  e3 = x
        em = self.layer4(x);  av4 = self.get_av(em)
