...
def getValue(img, lmk=[], type = 1, lmk_type=2, channels='BGR'):
    """Extract per-region mean values. img is BGR (cv2.imread); YUV conversion is disabled, so output channels are BGR."""
    Value = []
    # 1.三点对齐 2.两点对齐
    # 1.81点 2.68 点
    h, w, c = img.shape
...
def extract_frames_to_folder(video_path, out_folder, max_frames=None):
    """
    Extract video frames to out_folder as 00000.png, 00001.png, ... (ordered).
    Returns number of frames written. out_folder is created if needed.
    """
...
def mySTMap(imglist_root, lmk_all=[]):
    ...
    img_list = sorted(os.listdir(imglist_root))  # ordered: 00000.png, 00001.png, ...
    z = 0
    STMap = []
...
# %% [markdown]
# STMap generation for UBFC — run cells in order. Start with subject 1.
...
for z, subfile in enumerate(file_list):
    ...
    # Find video (.avi etc.) in raw subject directory
    video_path = find_video_path(raw_subject_path)
    ...
    if n_frames == 0:
        ...
        continue
    if n_frames != n_landmarks:
        print(f'  Warning: frame count {n_frames} != landmark rows {n_landmarks}; using min.')
    n_use = min(n_frames, n_landmarks)
    lmk_use = lmk_all[:n_use]
    # If we wrote more frames than we need, mySTMap uses sorted listdir; frame files are 00000.. so first n_use match lmk_use
    STMap = mySTMap(align_folder, lmk_all=lmk_use)  # STMap channels: BGR (from cv2.imread, no YUV)
    out_file = os.path.join(STMap_path, STMap_name)
    cv2.imwrite(out_file, STMap, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
    print('  ->', out_file)
# %%
