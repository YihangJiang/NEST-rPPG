...
#!/usr/bin/env python3
"""
Preprocessing script PART 1: Extract face landmarks from video.

Extract 2D face landmarks for every frame of a video and save as RGB_lmk.csv.

Output: ./Label/RGB_lmk.csv

Required environment: face_alignment, cv2, torch
"""
import os
import sys
import csv
...
def extract_landmarks_from_video(video_path, save_dir=None, device="cpu"):
    ...
    Returns:
        ...
        frame_idx: Number of frames processed
    """
...
csv_path, num_frames = extract_landmarks_from_video(
    TEST_VIDEO,
    save_dir=SCRIPT_DIR,
    device="cpu",
)
if csv_path:
    print("Done. Landmarks:", csv_path)
    print(f"Total frames processed: {num_frames}")
    # Save frame count to a file for the next script
    frame_count_file = os.path.join(SCRIPT_DIR, "Label", "frame_count.txt")
    with open(frame_count_file, "w") as f:
        f.write(str(num_frames))
    print(f"Frame count saved to: {frame_count_file}")
else:
    sys.exit(1)
...
print("=" * 60)
print("Next step: Run preprocess_label.py in the other conda environment")
