...
def plot_pixel_distribution(nest_img, my_img, session_name, save_dir, w_common):
    """
    Plot pixel value distribution comparison (histograms per channel) for NEST vs PURE_my.
    Uses first w_common frames from both. Saves to save_dir/<session>_pixel_distribution.png.
    """
    ...
    for ch, (ax, label) in enumerate(zip(axes, ch_names)):
        ax.hist(nest_c[:, :, ch].ravel(), bins=bins, alpha=0.6, label="NEST PURE", color="C0", density=True)
        ax.hist(my_c[:, :, ch].ravel(), bins=bins, alpha=0.6, label="PURE_my", color="C1", density=True)
        ax.set_xlabel("Pixel value")
        ax.set_ylabel("Density")
        ax.set_title(f"Channel {label}")
        ax.legend()
        ax.grid(True, alpha=0.3)
    fig.suptitle(f"{session_name} STMap pixel distribution (first {w_common} frames)")
    plt.tight_layout()
...
# %%
print("=" * 60)
print("STMap content comparison: NEST PURE vs PURE_my")
print("=" * 60)

nest_sessions = sorted(
    [d for d in os.listdir(NEST_PURE_ROOT) if not d.startswith(".") and os.path.isdir(os.path.join(NEST_PURE_ROOT, d))]
)
my_sessions = sorted(
    [d for d in os.listdir(PURE_MY_ROOT) if not d.startswith(".") and os.path.isdir(os.path.join(PURE_MY_ROOT, d))]
)
common = sorted(set(nest_sessions) & set(my_sessions))
print(f"Common sessions: {len(common)} (NEST: {len(nest_sessions)}, PURE_my: {len(my_sessions)})")

FIGURES_DIR = os.path.join(SCRIPT_DIR, "pure_stmap_figures")
...
for name in common:
    nest_path = os.path.join(NEST_PURE_ROOT, name)
    my_path = os.path.join(PURE_MY_ROOT, name)
    ...
    print(f"  PURE_my: shape={my_img.shape} (H={h_m}, time={w_m}, C={c_m})")
    ...
    for ch in range(3):
        ...
        if np.std(a) > 0 and np.std(b) > 0:
            ...
            corr.append(np.nan)
    print(f"  Over first {w} frames: MAE={mae:.4f}, max_diff={max_diff:.1f}, corr(B,G,R)={[f'{c:.4f}' for c in corr]}")
    if w_n != w_m:
        print(f"  -> Different frame count (NEST={w_n}, PURE_my={w_m}) -> different STMap width.")
    if mae > 1.0:
...
print("  - Different alignment or getValue type -> different pixel values.")
