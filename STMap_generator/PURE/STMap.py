# %%
"""STMap generation for PURE. Run after Landmark_PURE.py and Align_Face_PURE.py."""
import os
import math
import csv
import cv2
...
def mySTMap(imglist_root, lmk_all=[]):
    # Use pre-aligned frames (10000.png, 10001.png, ...) from Align_Face_PURE.py
...
# %%
# Run STMap generation: read Align + Label from PURE_my, write STMap to PURE_my
if not os.path.isdir(PURE_MY_ROOT):
    ...
    for sub_name in sorted(os.listdir(PURE_MY_ROOT)):
        if sub_name.startswith('.'):
            continue
        subject_path = os.path.join(PURE_MY_ROOT, sub_name)
